...
const fragmentShaderSource = `
  ...
  void main() {
    ...
    // Sample texture A
    float redA = texture2D(u_videoA, uv + chroma).r;
    float greenA = texture2D(u_videoA, uv).g;
    float blueA = texture2D(u_videoA, uv - chroma).b;
    vec3 colorA = vec3(redA, greenA, blueA);

    // Sample texture B
    float redB = texture2D(u_videoB, uv + chroma).r;
    float greenB = texture2D(u_videoB, uv).g;
    float blueB = texture2D(u_videoB, uv - chroma).b;
    vec3 colorB = vec3(redB, greenB, blueB);

    // Blend between lower and upper scene videos
    vec3 finalColor = mix(colorA, colorB, u_local);
    vec3 color = grade(finalColor, u_scene);
