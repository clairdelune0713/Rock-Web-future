...
const fragmentShaderSource = `
  ...
  void main() {
    ...
    // Diagonal sliding wipe transition to completely avoid double-exposure overlapping
    float tilt = 0.12 * (v_uv.y - 0.5);
    float boundary = 1.0 - u_local + tilt;
    float mask = smoothstep(boundary + 0.02, boundary - 0.02, v_uv.x);
...
export function createWebGLScene({
  ...
  // Set up textureA (unit 0) with a 1x1 solid dark fallback to avoid image flashes
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array([22, 15, 11, 255]));

  // Set up textureB (unit 1) with a 1x1 solid dark fallback to avoid image flashes
  ...
  function render(time) {
    ...
    // 1. Reset texture A to solid dark fallback if lower index changes (video buffering/loading)
    ...
    // 2. Reset texture B to solid dark fallback if upper index changes (video buffering/loading)
